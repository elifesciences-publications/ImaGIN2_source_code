    ...
    switch S.Method
        ...
        case{'Coherence'}
            ...
            try
                ...
                S.FrequencyRange = ...
                    spm_input('Frequency range [Hz] ([min max])', '+1', 'i','[]');
            end
    end
        ...
        case{'Morlet wavelet'}
            try
                ...
                switch tmp
                    ...
                    case{'Yes'}
                        S.FactMod=...
                            spm_input('Factor of modulation', '+1', 'r', '7', 1);
                end
            end
            ...
            try
                ...
                S.Mfactor = ...
                    spm_input('Morlet wavelet factor', '+1', 'r', '7', 1);
            end
            ...
            try
                ...
                S.Width = ...
                    spm_input('Number of oscillations for integration?', '+1', 'r', '5', 1);
            end
            ...
            if S.Width>0
                try
                    ...
                    S.Coarse = ...
                        spm_input('Downsampling factor for integration?', '+1', 'r', '5', 1);
                end
            else
                try
                    S.TimeResolution;
                    ...
                    S.TimeResolution = ...
                        spm_input('Time Resolution [s]', '+1', 'r', '.1', 1);
                end
                S.Coarse=0;
            end
...
%         D.tf.Synchro=FlagSynchro;
        ...
        try
            ...
            D.tf.FrequencyRange = ...
                spm_input('Frequency range [Hz] ([min max])', '+1', 'r','[]');
        end
...
% catch
%     D.tf.rm_baseline = ...
% 		spm_input('Removal of baseline', '+1', 'y/n', [1,0], 1);
% end
D.tf.rm_baseline = 0;
...
%         catch
%             D.tf.Width = ...
%                 spm_input('Time window width [sec]?', '+1', 'r', '4', 1);
%         end
        
        D.tf.Width=2*D.tf.nfft/D.fsample;
%         D.tf.Width=D.tf.nfft/D.fsample;
...
%D.badchannels=setdiff(1:D.nchannels,D.tf.channels);
...
% 
% D.scale = zeros(length(D.tf.channels), 1, 1, D.ntrials);
% D.datatype = 'int16';
...
switch D.tf.Method
    ...
    case{'Coherence'}
        ...
        if ~isempty(TimeWindow)
%             d=D(:,indsample(D,TimeWindow(1)):indsample(D,TimeWindow(end)),:);
%             D=clone(D, ['c1' Pre '_' D.fnamedat], [D.nchannels indsample(D,TimeWindow(end)) D.ntrials]);
%             D(:,:,:)=d;
%             d2=D2(:,indsample(D2,TimeWindow(1)):indsample(D2,TimeWindow(end)),:);
%             D2=clone(D2, ['c2' Pre '_' D2.fnamedat], [D2.nchannels indsample(D2,TimeWindow(end)) D2.ntrials]);
%             D2(:,:,:)=d2;
        ...
        elseif D.tf.Coarse>1
            ...
            D2.futurSample=D.futurSample;
%            d=D(:,1:Nsamples,:);
%             D=clone(D, ['c1' Pre '_' D.fnamedat], [D.nchannels Nsamples D.ntrials]);
%             D(:,:,:)=d;
%             d=D2(:,1:Nsamples,:);
%             D2=clone(D2, ['c2' Pre '_' D2.fnamedat], [D2.nchannels Nsamples D2.ntrials]);
%             D2(:,:,:)=d;
        end
        D.tf.Label='Power';
...
switch D.tf.Method
    case{'Morlet wavelet','Hilbert','Mexhat wavelet'}
      ...
      %  Evt=Dint.events;
       % Evt.time=(Evt.time-D.timeonset)/D.fsample;
        %???????????
%         for i1=1:length(Evt.time)
%             Evt(i1).time=min(find(abs(time-Evt(i1).time)==min(abs(time-Evt(i1).time))));
%         end
       % Dint=events(Dint,1,Evt);
        if FlagSynchro
            try
    ...
    case{'Coherence'}
        ...
        %         D.events.timesec=D.events.time;
        %         D.events.time=round(D.events.time/D.tf.Coarse);
        %         D.events.time(find(D.events.time==0))=1;
        %         D2.events.timesec=D.events.timesec;
        %         D2.events.time=D.events.time;
    case{'Multitaper'}
...
%Write new channels for power
% tmp=D.chanlabels;
% D=chanlabels(D,{});
% for i1=1:size(D.tf.channels,2)
%     D.chanlabels{i1}=tmp{D.tf.channels(i1)};
% end
% D.channels.eeg=1:size(D.tf.channels,2);
% D.channels.base=ones(size(D.tf.channels,2),1);
% D.channels.sens=ones(size(D.tf.channels,2),1);
% D.channels.calib=ones(size(D.tf.channels,2),1);
% D.channels.bad=[];
% Dint.channels=D.channels;
% %Write new channels for coupling
% tmp=D.channels;
% if FlagSynchro
%     D2.channels.name = {};
%     for i1=1:size(D.tf.CM,2)
%         D2.channels.name{i1}=[tmp.name{D.tf.CM(1,i1)} '/' tmp.name{D.tf.CM(2,i1)}];
%     end
%     D2.channels.eeg=1:size(D.tf.CM,2);
%     D2.channels.base=ones(size(D.tf.CM,2),1);
%     D2.channels.sens=ones(size(D.tf.CM,2),1);
%     D2.channels.calib=ones(size(D.tf.CM,2),1);
%     D2int.channels=D2.channels;
% end
...
Coarse=1;
% while D.fsample/Coarse>2*max(D.tf.frequencies)*(1+1/(2*Dsupr.tf.FactMod))
if D.tf.FactMod>0
...
%             disp([i length(Dsupr.tf.channels) j Dsupr.Nfrequencies])
...
%         for i2 = 1:length(TimeWindow)
%             if Dsupr.tf.Width>0
%                 win0=find(time>=(TimeWindow(i2)-TimeWindowWidth/2)&time<=(TimeWindow(i2)+TimeWindowWidth/2));
%                 win0=win0(1:Dsupr.tf.Coarse:end);
%             else
%                 win0=win(i2);
%             end
%                         
%             for i = 1 : Dsupr.Nfrequencies
%                 NBin=round(((Dsupr.tf.Width/Dsupr.tf.frequencies(i))*Dsupr.fsample)/2);
%                 WIN=-length(M{i})+min(win0):max(win0)+length(M{i});
%                 WIN=WIN(find(WIN>0&WIN<=nsamples(Dsupr)));
%                 [tmp11,WINIndex]=intersect(WIN,win0);
%                 tmp1=squeeze(Dsupr(Dsupr.tf.channels,WIN,k));
%                 for j = 1 : length(Dsupr.tf.channels)
%                     
%                     %             disp([i length(Dsupr.tf.channels) j Dsupr.Nfrequencies])
%                     
...
%                         FrequencyMin = Dsupr.tf.frequencies(i)*(1-1/(2*Dsupr.tf.FactMod));
%                         FrequencyMax = Dsupr.tf.frequencies(i)*(1+1/(2*Dsupr.tf.FactMod));
%                         tmp1(j,:)=ImaGIN_bandpassFilter(tmp1(j,:),Dsupr.fsample,FrequencyMin,FrequencyMax);
%                     end
%                     
%                     tmp = conv(tmp1(j,:), M{i});
%                     % time shift to remove delay
%                     tmp = tmp([1:length(WIN)] + (length(M{i})-1)/2);
%                     
%                     % power                    
%                     dint(j, i, i2) = mean(abs(tmp(WINIndex)));
%                 end
%             end
%         end
%     end
...
nsamplenew=floor(D.nsamples/Coarse);
nsamplenew=size(d,2);
% Dsupr=fsample(Dsupr,Dsupr.fsample/Coarse);
time=time(1:Coarse:end);


% M = ImaGIN_mexhat(Dsupr.fsample, Dsupr.tf.frequencies);
...
%     if D.tf.Coarse>1
%         Dint.tf.Sbaseline=1+floor(D.tf.Sbaseline/Dsupr.tf.Coarse);
%         if FlagSynchro
%             D2int.tf.Sbaseline=1+floor(Dsupr.tf.Sbaseline/Dsupr.tf.Coarse);
%         end
%     end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [D,D2]=ComputeMultitaper(D,k,TimeWindow,TimeWindowWidth,Pre)
...
% full
% res = spm_eeg_specest_mtmconvol(S, d(D.tf.channels,:), time);

% Channels one by one
% for j = 1 : length(D.tf.channels)
%     tmp = spm_eeg_specest_mtmconvol(S, d(D.tf.channels(j),:), time);
%     if j==1
%         res = tmp;
%     else
%         res.fourier=[res.fourier;tmp.fourier];
%     end
% end
...
%     win=find(abs(time+(1e-3*S.timeres)-1e-3*timeoi((i1-1)*Nt+1))==min(abs(time+(1e-3*S.timeres)-1e-3*timeoi((i1-1)*Nt+1)))):...
%         find(abs(time-(1e-3*S.timeres)-1/fsample(D)-1e-3*timeoi(min([Nt*i1 length(timeoi)])))==min(abs(time-(1e-3*S.timeres)-1/fsample(D)-1e-3*timeoi(min([Nt*i1 length(timeoi)])))));
...
NBin=round(D.tf.Width*D.fsample/2);
...
if isempty(TimeWindow)
    ...
    for j = 1 : length(D.tf.channels)
        ...
        for i2 = 1:length(TimeWindow)
            ...
            %         d(j, :, n) = pwelch(tmp1(win),D.tf.nfft,D.fsample,[],[],0,'squared');
            tmp = sqrt(pwelch(tmp1(win).*hanning(length(win))',D.tf.nfft,D.fsample,[],[],0,'squared'));
            tmp=tmp(D.tf.frequenciesIndex);
            d(j, :, i2) = tmp;
        end
...
% D.scale(:, 1, 1, k) = (max(abs(reshape(d, [D.Nfrequencies*D.nsamples length(D.tf.channels)])))./(2^31-1))';
% D.scale(:, 1, 1, k) = max(max(d,[],3),[],2)./(2^15-1);
% d = int16(d./repmat(D.scale(:, 1, 1, k), [1 D.Nfrequencies D.nsamples]));
% fwrite(fpd, d, 'int16');
% D.datatype='int16';
if k==1
...
if FlagSynchro
    ...
    %     D2.scale(:, 1, 1, k) = max(max(d2,[],3),[],2)./32767;
    %     d2 = int16(d2./repmat(D2.scale(:, 1, 1, k), [1 D2.Nfrequencies D2.nsamples]));
